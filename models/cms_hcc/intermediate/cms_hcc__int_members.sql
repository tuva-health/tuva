...
with stg_eligibility as (
    ...
    select
        ...
        , {{ dbt_utils.concat(['dates.payment_year', "'-12-31'"]) }} as payment_year_end_date
        , row_number() over(
    ...
    inner join {{ ref('cms_hcc__int_monthly_collection_dates') }} as dates
        ...
        on elig.enrollment_start_date <= cast({{ dbt_utils.concat(['dates.payment_year', "'-12-31'"]) }} as date)
...
, payment_year_age_dates as (
    ...
    select distinct
          payment_year
        , cast({{ dbt_utils.concat(['payment_year',"'-02-01'"]) }} as date) as payment_year_age_date
