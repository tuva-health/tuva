...
{% set diagnosis_code_type_valid_where = "cast(source_rows.primary_diagnosis_code_type as " ~ string_type ~ ") in ('icd-10-cm', 'icd-9-cm')" %}
{% set diagnosis_code_type_invalid_where = "source_rows.primary_diagnosis_code_type is not null and cast(source_rows.primary_diagnosis_code_type as " ~ string_type ~ ") not in ('icd-10-cm', 'icd-9-cm')" %}
...
final as (
    select
        ...
        , {{ dq_logical_int_flag_sql("source_rows.encounter_type is not null and encounter_type_lookup.encounter_type is null", "source_rows.encounter_type is not null") }} as encounter_type_invalid
        , {{ dq_logical_int_flag_sql("source_rows.encounter_start_date is null", "1 = 1") }} as encounter_start_date_null
        , {{ dq_logical_int_flag_sql("source_rows.encounter_end_date is null", "1 = 1") }} as encounter_end_date_null
        , {{ dq_logical_int_flag_sql(
              "source_rows.encounter_start_date is not null and source_rows.encounter_end_date is not null and source_rows.encounter_start_date > source_rows.encounter_end_date",
              "source_rows.encounter_start_date is not null and source_rows.encounter_end_date is not null"
          ) }} as encounter_start_date_after_encounter_end_date
        , {{ dq_logical_int_flag_sql(
              "source_rows.encounter_start_date is not null and (source_rows.encounter_start_date < " ~ min_encounter_date_sql ~ " or source_rows.encounter_start_date > " ~ current_date_sql ~ ")",
              "source_rows.encounter_start_date is not null"
          ) }} as encounter_start_date_out_of_reasonable_range
        , {{ dq_logical_int_flag_sql(
        ...
        , {{ dq_logical_int_flag_sql("source_rows.facility_npi is not null and facility_provider_lookup.npi is null", "source_rows.facility_npi is not null") }} as facility_npi_invalid
        , {{ dq_logical_int_flag_sql(
              "source_rows.primary_diagnosis_code_type is null",
              "source_rows.primary_diagnosis_code is not null"
          ) }} as primary_diagnosis_code_type_null
        , {{ dq_logical_int_flag_sql(diagnosis_code_type_invalid_where, "source_rows.primary_diagnosis_code_type is not null") }} as primary_diagnosis_code_type_invalid
        , {{ dq_logical_int_flag_sql(
              "source_rows.primary_diagnosis_code is null",
              "source_rows.primary_diagnosis_code_type is not null"
          ) }} as primary_diagnosis_code_null
        , {{ dq_logical_int_flag_sql(
              "source_rows.primary_diagnosis_code is not null "
              ~ "and " ~ diagnosis_code_type_valid_where ~ " "
              ~ "and icd_10_cm_lookup.icd_10_cm is null "
              ~ "and icd_9_cm_lookup.icd_9_cm is null",
              "source_rows.primary_diagnosis_code is not null and " ~ diagnosis_code_type_valid_where
          ) }} as primary_diagnosis_code_invalid
        , {{ dq_logical_int_flag_sql(
              "source_rows.drg_code_type is null",
              "source_rows.drg_code is not null"
          ) }} as drg_code_type_null
        , {{ dq_logical_int_flag_sql(drg_code_type_invalid_where, "source_rows.drg_code_type is not null") }} as drg_code_type_invalid
        , {{ dq_logical_int_flag_sql(
        ...
        , {{ dq_logical_int_flag_sql(
              ...
              "source_rows.drg_code is not null and " ~ drg_code_type_valid_where
          ) }} as drg_code_invalid
    from source_rows
    ...
    left join {{ ref('terminology__encounter_type') }} as encounter_type_lookup
        on cast(source_rows.encounter_type as {{ string_type }}) = cast(encounter_type_lookup.encounter_type as {{ string_type }})
    ...
    left join {{ ref('terminology__icd_10_cm') }} as icd_10_cm_lookup
        on cast(source_rows.primary_diagnosis_code_type as {{ string_type }}) = 'icd-10-cm'
    ...
    left join {{ ref('terminology__icd_9_cm') }} as icd_9_cm_lookup
        on cast(source_rows.primary_diagnosis_code_type as {{ string_type }}) = 'icd-9-cm'
