...
select
    ...
    , 'NORMALIZED_ORDER_TYPE' as field_name
    , case when term.code_type is not null then 'valid'
          when m.normalized_order_type is not null then 'invalid'
          else 'null'
    end as bucket_name
    , case when m.normalized_order_type is not null and term.code_type is null
