...
select
    ...
    , case
        when cast(m.result_datetime as {{ dbt.type_timestamp() }} ) > cast('{{ var('tuva_last_run') }}' as {{ dbt.type_timestamp() }} ) then 'invalid'
        when cast(m.result_datetime as {{ dbt.type_timestamp() }} ) <= cast('1900-01-01 00:00:00' as {{ dbt.type_timestamp() }} ) then 'invalid'
        when m.result_datetime < m.collection_datetime then 'invalid'
        when m.result_datetime is null then 'null'
        else 'valid'
    end as bucket_name
    , case
        when cast(m.result_datetime as {{ dbt.type_timestamp() }} ) > cast('{{ var('tuva_last_run') }}' as {{ dbt.type_timestamp() }} ) then 'future'
        when cast(m.result_datetime as {{ dbt.type_timestamp() }} ) <= cast('1900-01-01 00:00:00' as {{ dbt.type_timestamp() }} ) then 'too old'
