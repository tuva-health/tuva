...
select h.data_source,
  h.payer, h.{{ quote_column('plan') }},
  cast(null as {{ dbt.type_string() }}) as year_month,
  'hcc_count' as metric,
  'hcc_code' as dim1_name,
  h.hcc_code as dim1_value,
  coalesce(d.hcc_description, h.hcc_code) as dim1_label,
  cast(null as {{ dbt.type_string() }}) as dim2_name,
  cast(null as {{ dbt.type_string() }}) as dim2_value,
  cast(null as {{ dbt.type_string() }}) as dim2_label,
  cast(h.count_members as {{ dbt.type_numeric() }}) as value,
  h.tuva_last_run as tuva_last_run
from {{ ref('mart_review__hcc_counts') }} h
left join (
  select hcc_code, max(hcc_description) as hcc_description
  from {{ ref('cms_hcc__int_disease_factors') }}
  group by hcc_code
) d on h.hcc_code = d.hcc_code
