...
with range_cte as (
  select min(claim_start_date) as first_date
...
inner join range_cte r on c.full_date between r.first_date and r.last_date
)
...
from {{ ref('medical_claim')}} p 
left join {{ ref('reference_data__calendar')}} c on p.claim_start_date = c.full_date
group by c.year_month_int
)
...
from date_cte d
left join medical_claim m on d.year_month_int = m.year_month_int
