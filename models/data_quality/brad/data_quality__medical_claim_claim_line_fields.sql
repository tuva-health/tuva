...
,max(case when claim_type = 'professional' and pos.place_of_service_code is null and m.place_of_service_code is not null then 1 else 0 end) as invalid_place_of_service_code
...
left join {{ ref('terminology__hcpcs_level_2')}} as hcpc on m.hcpcs_code = hcpc.hcpcs
left join {{ ref('terminology__place_of_service')}} as pos on m.place_of_service_code = pos.place_of_service_code
left join {{ ref('terminology__revenue_center')}} as rev on m.revenue_center_code = rev.revenue_center_code
group by claim_id
)

select 'invalid hcpcs_code' as data_quality_check
,sum(invalid_hcpcs_code) as result_count
from medical_claim

union all

select 'invalid place_of_service_code' as data_quality_check
,sum(invalid_place_of_service_code) as result_count
from medical_claim

union all

select 'invalid revenue_center_code' as data_quality_check
,sum(invalid_revenue_center_code) as result_count
from medical_claim

union all

select 'missing hcpcs_code' as data_quality_check
,sum(missing_hcpcs_code) as result_count
from medical_claim

union all

select 'missing place_of_service_code' as data_quality_check
,sum(missing_place_of_service_code) as result_count
from medical_claim

union all

select 'missing revenue_center_code' as data_quality_check
