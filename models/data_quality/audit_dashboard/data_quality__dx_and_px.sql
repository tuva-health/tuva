...
, claims_with_primary_dx as(
    select
        claim_id
        , count(case when diagnosis_code_1 is not null then 1 else 0 end) as primary_dx_populated
    from {{ ref('medical_claim') }}
    group by
        claim_id
)
, claims_with_secondary_dx as(
    select
        claim_id
        , count(case when diagnosis_code is not null then 1 else 0 end) as secondary_dx_populated
...
, secondary_populated_missing_primary as(
select
    sum(case when pdx.claim_id is null then 1 else 0 end) as missing_primary_count
from claims_with_secondary_dx sdx
left join claims_with_primary_dx pdx
    on pdx.claim_id = sdx.claim_id
)
...
, final as (
    ...
    union all
    select
     'primary dx missing but primary dx populated' as data_quality_check
    , sum(missing_primary_count) as result_count
    from secondary_populated_missing_primary
)
