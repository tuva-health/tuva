...
-- We want to show all claims that have a claim_start_date
-- with or without data quality problems.
-- We will have an output table at the claim_id grain and for
-- each claim_id we show:
--    [a] claim_start_date_always_populated ('Yes' or 'No')
--    [b] claim_start_date_unique ('Yes' or 'No')


-- NB:
-- We edit this file to remove the requirement [a], since
-- there might be datasets where claim_start_date is not available
...
claim_start_date_not_always_populated as (
select distinct claim_id
from all_rows
...
from all_rows
group by claim_id
having claim_start_date_count > 1
),
...
select
  ...
  -- end as claim_start_date_always_populated,

  case
    when cc.claim_id is not null then 'No'
    when cc.claim_id is null then 'Yes'
  end as claim_start_date_unique

from all_claim_ids aa

left join claim_start_date_not_always_populated bb
on aa.claim_id = bb.claim_id

left join claim_start_date_not_unique cc
