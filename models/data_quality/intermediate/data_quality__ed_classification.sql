...
select 'missing ed_care_needed_preventable' as data_quality_check,
       case when ed_care_needed_preventable = 0 then 1 else 0 end as result_count
from cte

union all

select 'missing pc_treatable' as data_quality_check,
       case when pc_treatable = 0 then 1 else 0 end as result_count
from cte

union all

select 'missing ed_care_needed_not_preventable' as data_quality_check,
       case when ed_care_needed_not_preventable = 0 then 1 else 0 end as result_count
from cte

union all

select 'missing non_emergent' as data_quality_check,
