...
UNIQUE_FIELD as (
    SELECT DISTINCT CLAIM_ID
        ...
        ,DATA_SOURCE
    FROM BASE
    LEFT JOIN {{ source('tuva_terminology','admit_source') }} AS TERM ON BASE.ADMIT_SOURCE_CODE = TERM.ADMIT_SOURCE_CODE
),
CLAIM_GRAIN as (
    SELECT CLAIM_ID
        ,DATA_SOURCE
...
LEFT JOIN {{ source('tuva_terminology','admit_source') }} AS TERM ON M.ADMIT_SOURCE_CODE = TERM.ADMIT_SOURCE_CODE
