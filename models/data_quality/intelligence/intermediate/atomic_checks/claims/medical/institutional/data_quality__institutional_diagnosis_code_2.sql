...
UNIQUE_FIELD as (
    SELECT DISTINCT CLAIM_ID
        ,Diagnosis_Code_2 || '|' || coalesce(TERM.LONG_DESCRIPTION,'') as Field
        ,DATA_SOURCE
    FROM BASE
    LEFT JOIN {{ source('tuva_terminology','icd_10_cm') }} AS TERM ON BASE.Diagnosis_Code_2 = TERM.ICD_10_CM
),
CLAIM_GRAIN as (
    SELECT CLAIM_ID
        ,DATA_SOURCE
...
LEFT JOIN {{ source('tuva_terminology','icd_10_cm') }} AS TERM ON M.Diagnosis_Code_2 = TERM.ICD_10_CM
