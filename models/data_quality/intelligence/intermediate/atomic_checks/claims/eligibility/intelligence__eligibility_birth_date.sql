...
SELECT DISTINCT
    ...
    ,CASE 
        ...
        WHEN M.BIRTH_DATE > CURRENT_DATE() THEN 'invalid'
        WHEN M.BIRTH_DATE <= '1901-01-01' THEN 'invalid'
        WHEN M.BIRTH_DATE IS NULL THEN 'null'
        ELSE 'valid' 
    END AS BUCKET_NAME
    ,CASE 
        WHEN CG.FREQUENCY > 1 THEN 'multiple'      
        WHEN M.BIRTH_DATE > CURRENT_DATE() THEN 'future'
        WHEN M.BIRTH_DATE <= '1901-01-01' THEN 'too old'
        else null
