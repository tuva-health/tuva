...
with add_row_number_to_claims as (
    ...
    select
          *
        , row_number() over (partition by patient_id, encounter_id order by merge_start_date, merge_end_date, claim_id) as row_num_first
        , row_number() over (partition by patient_id, encounter_id order by merge_end_date, merge_start_date, claim_id desc) as row_num_last
