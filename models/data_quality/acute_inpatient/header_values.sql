...
from {{ ref('claim_type') }} claim_type

left join {{ ref('bill_type_code_summary') }} bill
    on claim_type.claim_id = bill.claim_id

left join {{ ref('ms_drg_code_summary') }} ms
    on bill.claim_id = ms.claim_id

left join {{ ref('apr_drg_code_summary') }} apr
    on bill.claim_id = apr.claim_id

left join {{ ref('admit_type_code_summary') }} atco
    on bill.claim_id = atco.claim_id

left join {{ ref('admit_source_code_summary') }} asco
    on bill.claim_id = asco.claim_id

left join {{ ref('discharge_disposition_code_summary') }} ddco
