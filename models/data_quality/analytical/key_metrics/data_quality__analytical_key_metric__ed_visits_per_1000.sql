{{ dq_config_analytical_metric_model('analytical_key_metric__ed_visits_per_1000') }}
...
{% if execute and core_encounter_rel is not none and core_member_months_rel is not none %}
    select
        ...
        , {{ dq_analytical_decimal_result_sql(
            "case when coalesce(member_month_totals.member_months, 0) = 0 then 0 else (cast(coalesce(ed_counts.encounter_count, 0) as " ~ dbt.type_numeric() ~ ") / cast(member_month_totals.member_months as " ~ dbt.type_numeric() ~ ")) * 12000 end"
          ) }} as result
    from (
