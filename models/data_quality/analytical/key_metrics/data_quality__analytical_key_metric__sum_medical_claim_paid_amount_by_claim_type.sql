{{ dq_config_analytical_metric_model('analytical_key_metric__sum_medical_claim_paid_amount_by_claim_type') }}
...
{% if execute and core_medical_claim_rel is not none %}
    select
        ...
        , {{ dq_analytical_decimal_result_sql("coalesce(sum(paid_amount), 0)") }} as result
