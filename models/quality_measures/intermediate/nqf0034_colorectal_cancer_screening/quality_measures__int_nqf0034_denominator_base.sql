...
,
    ...
    inner join performance_period as pp on 1=1
...
/*    and coalesce(ENCOUNTER.ENCOUNTER_END_DATE,ENCOUNTER.ENCOUNTER_END_DATE) >=  cast({{ performance_period_begin }} as date)
        and coalesce(ENCOUNTER.ENCOUNTER_START_DATE,ENCOUNTER.ENCOUNTER_END_DATE) <= cast('{{ performance_period_end }}' as date)
*/
...
/*
          where coalesce(MEDICAL_CLAIM.CLAIM_END_DATE,MEDICAL_CLAIM.CLAIM_START_DATE) >=  cast({{ performance_period_begin }} as date)
         and coalesce(MEDICAL_CLAIM.CLAIM_END_DATE,MEDICAL_CLAIM.CLAIM_END_DATE) <= cast('{{ performance_period_end }}' as date)
*/
)
