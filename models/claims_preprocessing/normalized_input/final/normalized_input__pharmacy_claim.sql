...
select
    ...
    , cast(coalesce({{ dbt_utils.concat(["pres.provider_last_name", "', '", "pres.provider_first_name"]) }}, pres.provider_organization_name) as {{ dbt.type_string() }} ) as prescribing_provider_name
    , cast(dispensing_provider_npi as {{ dbt.type_string() }} ) as dispensing_provider_id
    , cast(coalesce({{ dbt_utils.concat(["disp.provider_last_name", "', '", "disp.provider_first_name"]) }}, disp.provider_organization_name) as {{ dbt.type_string() }} ) as dispensing_provider_name    , cast(dispensing_date as date ) as dispensing_date
