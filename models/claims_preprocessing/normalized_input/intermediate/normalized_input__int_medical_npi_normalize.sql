...
select distinct
  ...
  , case 
      when rend_prov.entity_type_code = '1' then 
        cast({{ dbt_utils.concat(["rend_prov.provider_last_name", "', '", "rend_prov.provider_first_name"]) }} as {{ dbt.type_string() }})
      else 
  ...
  , case 
      when bill_prov.entity_type_code = '1' then
        cast({{ dbt_utils.concat(["bill_prov.provider_last_name", "', '", "bill_prov.provider_first_name"]) }} as {{ dbt.type_string() }})
      else 
  ...
  , case 
      when fac_prov.entity_type_code = '1' then
        cast({{ dbt_utils.concat(["fac_prov.provider_last_name", "', '", "fac_prov.provider_first_name"]) }} as {{ dbt.type_string() }})
      else 
