...
from {{ ref('int_service_category__office_based') }} as med
  left outer join {{ ref('int_service_category__pharmacy_professional') }} as pharm
  on med.claim_line_id = pharm.claim_line_id
  and med.data_source = pharm.data_source
  left outer join {{ ref('int_service_category__office_based_radiology_professional') }} as rad
  on med.claim_line_id = rad.claim_line_id
  and med.data_source = rad.data_source
  left outer join {{ ref('int_service_category__office_based_visit_professional') }} as visit
  on med.claim_line_id = visit.claim_line_id
  and med.data_source = visit.data_source
  left outer join {{ ref('int_service_category__office_based_surgery_professional') }} as surg
  on med.claim_line_id = surg.claim_line_id
  and med.data_source = surg.data_source
  left outer join {{ ref('int_service_category__office_based_therapy_professional') }} as pt
