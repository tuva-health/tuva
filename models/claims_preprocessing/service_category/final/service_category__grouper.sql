...
with service_category_1_mapping as (
    ...
    left outer join {{ ref('int_service_category__combined_professional') }} as b
    ...
    select distinct
        ...
        , b.source_model_name
    from {{ ref('stg_service_category__medical_claim') }} as a
    left outer join {{ ref('int_service_category__combined_institutional_header_level') }} as b
      on a.claim_id = b.claim_id
      and a.data_source = b.data_source
    left outer join {{ ref('service_category__service_categories') }} as s
      on b.service_category_1 = s.service_category_1
    ...
    select distinct
        ...
        , b.source_model_name
    from {{ ref('stg_service_category__medical_claim') }} as a
    left outer join {{ ref('int_service_category__combined_institutional_line_level') }} as b
      on a.claim_id = b.claim_id
      and a.claim_line_number = b.claim_line_number
      and a.data_source = b.data_source
    left outer join {{ ref('service_category__service_categories') }} as s
...
inner join {{ ref('stg_service_category__medical_claim') }} as s
