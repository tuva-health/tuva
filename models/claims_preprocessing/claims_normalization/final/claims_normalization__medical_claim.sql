...
left outer join {{ ref('claims_normalization__int_admit_source_final') }} as ad_source
    on med.claim_id = ad_source.claim_id
    and med.data_source = ad_source.data_source
left outer join {{ ref('claims_normalization__int_admit_type_final') }} as ad_type
    on med.claim_id = ad_type.claim_id
    and med.data_source = ad_type.data_source
left outer join {{ ref('claims_normalization__int_bill_type_final') }} as bill
    on med.claim_id = bill.claim_id
    and med.data_source = bill.data_source
left outer join {{ ref('claims_normalization__int_medical_claim_date_normalize') }} as claim_line_dates
...
left outer join {{ ref('claims_normalization__int_medical_date_aggregation') }} as dates
    on med.claim_id = dates.claim_id
    and med.data_source = dates.data_source
left outer join {{ ref('claims_normalization__int_medical_npi_normalize') }} as med_npi
...
left outer join {{ ref('claims_normalization__int_discharge_disposition_final') }} as disch_disp
    on med.claim_id = disch_disp.claim_id
    and med.data_source = disch_disp.data_source
left outer join {{ ref('claims_normalization__int_drg_final') }} as drg
    on med.claim_id = drg.claim_id
    and med.data_source = drg.data_source
left outer join {{ ref('claims_normalization__int_place_of_service_normalize') }} as pos
    on med.claim_id = pos.claim_id
    and med.claim_line_number = pos.claim_line_number
    and med.data_source = pos.data_source
left outer join {{ ref('claims_normalization__int_diagnosis_code_final') }} as dx_code
    on med.claim_id = dx_code.claim_id
    and med.data_source = dx_code.data_source
left outer join {{ ref('claims_normalization__int_present_on_admit_final') }} as poa
    on med.claim_id = poa.claim_id
    and med.data_source = poa.data_source
left outer join {{ ref('claims_normalization__int_procedure_code_final') }} as px_code
    on med.claim_id = px_code.claim_id
    and med.data_source = px_code.data_source
left outer join {{ ref('claims_normalization__int_procedure_date_final') }} as px_date
    on med.claim_id = px_date.claim_id
    and med.data_source = px_date.data_source
left outer join {{ ref('claims_normalization__int_revenue_center_normalize') }} as rev
    on med.claim_id = rev.claim_id
    and med.claim_line_number = rev.claim_line_number
    and med.data_source = rev.data_source
left outer join {{ ref('claims_normalization__int_undetermined_claim_type') }} as undetermined
