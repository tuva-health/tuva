...
order by stg.claim_type, stg.start_date) as encounter_row_number --institutional then professional
    from {{ ref('encounter_grouper__stg_medical_claim') }} as stg
    ...
    inner join {{ ref('asc__start_end_dates') }} as ed on cli.old_encounter_id = ed.old_encounter_id
)
...
, patient as (
    select
        ...
        , gender
        , race
    from {{ ref('encounter_grouper__stg_eligibility') }}
...
, service_category_flags as (
    ...
    left outer join {{ ref('service_category_grouper__service_category_grouper') }} as scr on d.claim_id = scr.claim_id
    and
