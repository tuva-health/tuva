...
from {{ ref('encounter_grouper__stg_medical_claim') }} as med
inner join {{ ref('encounter_grouper__stg_professional') }} as prof on med.claim_line_id = prof.claim_line_id
...
from {{ ref('encounter_grouper__stg_medical_claim') }} as med
inner join {{ ref('encounter_grouper__stg_outpatient_institutional') }} as inst on med.claim_id = inst.claim_id
inner join join_first_claim_dates as dat on med.patient_data_source_id = dat.patient_data_source_id
and med.start_date between dat.encounter_start_date and dat.encounter_end_date
