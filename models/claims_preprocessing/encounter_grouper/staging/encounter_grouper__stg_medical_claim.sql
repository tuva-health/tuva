...
, final as (
  select
    ...
    , m.allowed_amount
    , m.data_source
  from {{ ref('claims_normalization__medical_claim') }} as m
  inner join {{ ref('service_category_grouper__service_category_grouper') }} as g on m.claim_id = g.claim_id
    and m.claim_line_number = g.claim_line_number
    and g.duplicate_row_number = 1
  inner join {{ ref('encounter_grouper__patient_data_source_id') }} as d on m.person_id = d.person_id
