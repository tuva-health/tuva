...
, final as (
  ...
  left outer join {{ ref('stg_service_category__claim_level_diagnosis') }} as claim_dx
    on m.claim_id = claim_dx.claim_id
    and m.data_source = claim_dx.data_source
  inner join {{ ref('service_category__grouper') }} as g on m.claim_id = g.claim_id
    and m.claim_line_number = g.claim_line_number
    and g.duplicate_row_number = 1
  inner join {{ ref('normalized__patient_data_source_id') }} as d on m.person_id = d.person_id
