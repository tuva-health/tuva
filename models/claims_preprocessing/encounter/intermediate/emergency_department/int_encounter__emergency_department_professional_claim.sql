...
with first_claim as (
    select *
    from {{ ref('int_encounter__emergency_department_generate_id') }}
...
, join_first_claim_dates as (
    ...
    inner join {{ ref('int_encounter__emergency_department_start_end_date') }} as dat on f.encounter_id = dat.encounter_id
)
...
inner join {{ ref('int_encounter__professional_service_type') }} as prof on med.claim_line_id = prof.claim_line_id
...
from {{ ref('int_encounter__claim_line') }} as med
inner join {{ ref('int_encounter__outpatient_institutional_service_type') }} as inst on med.claim_id = inst.claim_id
inner join join_first_claim_dates as dat on med.patient_data_source_id = dat.patient_data_source_id
and med.start_date between dat.encounter_start_date and dat.encounter_end_date
