...
select
  ...
  , sum(case when total_units > 0 then paid_amount else null end)/sum(case when total_units > 0 then total_units else null end) as generic_average_cost_per_unit
