...
select
    ...
    {{ select_extension_columns(ref('int_normalized__eligibility_casting'), alias='elig', strip_prefix=false) }}
    , elig.file_date
    , elig.file_name
    , elig.ingest_datetime
    , elig.tuva_last_run
    , elig.data_source
from {{ ref('int_normalized__eligibility_casting') }} as elig
left outer join {{ ref('int_normalized__eligibility_date_normalized') }} as date_norm
  on elig.person_id_key = date_norm.person_id_key
left outer join {{ ref('int_normalized__eligibility_state_normalized') }} as state_norm
