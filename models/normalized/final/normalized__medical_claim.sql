...
select
    ...
    {{ select_extension_columns(ref('stg_input_layer__medical_claim'), alias='med', strip_prefix=false) }}
from {{ ref('stg_normalized__medical_claim') }} as med
left outer join {{ ref('int_normalized__admit_source_final') }} as ad_source
    on med.claim_id = ad_source.claim_id
    and med.data_source = ad_source.data_source
left outer join {{ ref('int_normalized__admit_type_final') }} as ad_type
    on med.claim_id = ad_type.claim_id
    and med.data_source = ad_type.data_source
left outer join {{ ref('int_normalized__bill_type_final') }} as bill
    on med.claim_id = bill.claim_id
    and med.data_source = bill.data_source
left outer join {{ ref('int_normalized__medical_claim_date_normalize') }} as claim_line_dates
...
left outer join {{ ref('int_normalized__medical_date_aggregation') }} as dates
    on med.claim_id = dates.claim_id
    and med.data_source = dates.data_source
left outer join {{ ref('int_normalized__medical_npi_normalize') }} as med_npi
