...
left outer join {{ ref('terminology__calendar') }} as cal_dob
    on elig.birth_date = cal_dob.full_date
left outer join {{ ref('terminology__calendar') }} as cal_death
    on elig.death_date = cal_death.full_date
left outer join {{ ref('terminology__calendar') }} as cal_enroll_start
    on elig.enrollment_start_date = cal_enroll_start.full_date
left outer join {{ ref('terminology__calendar') }} as cal_enroll_end
