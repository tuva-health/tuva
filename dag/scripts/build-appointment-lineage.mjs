...
const repoRoot = path.resolve(dagRoot, "..");
const manifestPath = path.join(repoRoot, "integration_tests", "target", "manifest.json");
...
export function getOutputPathForTarget(targetKey = DEFAULT_TARGET_KEY) {
  return path.join(dagRoot, "data", `${targetKey}-lineage.json`);
}
...
function resolveInputLayerEntryName(manifestNode) {
  ...
  if (normalizedPath.startsWith("models/input_layer/")) {
