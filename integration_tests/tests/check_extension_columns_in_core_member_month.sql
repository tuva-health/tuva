...
-- present and correctly populated in core.member_month.
--
...
--      core.member_month (SQL fails to compile → caught as an error).
...
-- depends_on: {{ ref('core__member_month') }}
...
{%- set source_relation = ref('stg_input_layer__eligibility') -%}
{%- set extension_cols = [] -%}
...
where
    ...
    and (select count(*) from {{ ref('core__member_month') }}) > 0
