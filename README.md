...
Organizations that have mapped all their healthcare claims data to the Tuva Claims Data Model can easily run the entire Tuva Project (all 5 dbt packages) by using the the_tuva_project package. You can think of this package (i.e. the_tuva_project package) as a meta-package that is used to run all dbt packages that are part of the Tuva Project with one command. This package calls all 5 packages and runs them all while handling the dependencies between them. The input layer for the_tuva_project is the entire Tuva Claims Data Model. This means that to run the entire Tuva Project (i.e. the_tuva_project package), you need to map all of your claims data into the [Tuva Claims Data Model](https://thetuvaproject.com/docs/the-tuva-project/claims-data-model/eligibility).
