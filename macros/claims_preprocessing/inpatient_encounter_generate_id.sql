{% macro inpatient_encounter_generate_id(service_category_2) %}
{#
    Body shared by every institutional inpatient *_generate_id model.

    Seven models -- acute inpatient, hospice, long term, psychiatric, rehab,
    skilled nursing, substance use -- were byte-identical apart from one
    service_category_2 value. They stitch a patient's institutional claims into
    stays by walking claims in date order, merging any claim that overlaps,
    abuts a transfer, or duplicates the one before it, and naming each stay
    after the claim that closes it.

    The four merge conditions were identical across all seven and are defined
    here once. A change to the stitching rules is now a change in one file.

    Arguments:
        service_category_2: the value that selects this inpatient type,
                            e.g. 'inpatient hospice'
#}
