{% macro inpatient_encounter_final(key) %}
{#
    Body shared by every institutional inpatient encounter__* final model.

    Six models -- hospice, long term, psychiatric, rehabilitation, skilled
    nursing and substance use -- were identical apart from the encounter_type
    they select, the start_end_date model they read, and one column that only
    skilled nursing carries. They roll a stay's attributed claim lines up to one
    row per encounter: length of stay, admit and discharge coding, DRG, totals,
    the highest-paid diagnosis and facility, and service-category flags.

    Acute inpatient is deliberately not here -- it computes delivery, newborn,
    NICU and ED flags the others do not have.

    Arguments:
        key: an encounter_definitions() key, e.g. 'inpatient_hospice'.
             Its snf_part_b entry adds the column only skilled nursing reports.
#}
{%- set enc = encounter_definition(key) -%}
{%- set snf_part_b = enc.get('snf_part_b', false) -%}
...
with detail_values as (
    ...
    inner join {{ ref('int_encounter__combined_claim_line_crosswalk') }} as cli on stg.claim_id = cli.claim_id  --replace this ref with the deduped version when complete
    and
    stg.claim_line_number = cli.claim_line_number
    and
    cli.encounter_type = '{{ enc.encounter_type }}'
    and
    cli.claim_line_attribution_number = 1
    inner join {{ ref(enc.source_model) }} as ed on cli.old_encounter_id = ed.encounter_id
)
...
, patient as (
    select
        ...
        , sex
        , race
    from {{ ref('stg_encounter__eligibility') }}
...
, service_category_flags as (
    select
       ...
       {%- endif %}
    from detail_values as d
    left outer join {{ ref('service_category__grouper') }} as scr on d.claim_id = scr.claim_id
    and
...
from {{ ref(enc.source_model) }} as a
