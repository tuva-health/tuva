{% macro outpatient_encounter_final(key) %}
{#
    Body shared by every outpatient-shaped encounter__* final model.

    Eight models -- outpatient hospice, hospital or clinic, psychiatric,
    rehabilitation, substance use, surgery, therapy, and urgent care -- were
    identical apart from the encounter_type they select and the generate_id
    model they read. They roll a set of attributed claim lines up to one row per
    encounter: totals, the highest-paid diagnosis and facility, service-category
    flags, and patient demographics.

    Encounter types that carry extra columns (a length of stay, an admit source,
    a part B flag) do not use this macro -- they have their own body.

    Arguments:
        key: an encounter_definitions() key, e.g. 'outpatient_hospice'
#}
